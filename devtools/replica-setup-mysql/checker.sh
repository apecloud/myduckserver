...
check_user_privileges() {
    echo "Checking privileges for the current user '$SOURCE_HOST'..."

    # Check the user grants for the currently authenticated user using mysqlsh
    result=$(mysqlsh --host="$SOURCE_HOST" --port="$SOURCE_HOST" --user="$SOURCE_HOST" --password="$SOURCE_PASSWORD" --sql -e "
    ...
    if echo "$result" | grep -q -E "GRANT (SELECT|RELOAD|REPLICATION CLIENT|REPLICATION SLAVE|SHOW VIEW|EVENT)"; then
        echo "Current user '$SOURCE_HOST' has all required privileges."
    elif echo "$result" | grep -q "GRANT ALL"; then
        echo "Current user '$SOURCE_HOST' has 'GRANT ALL' privileges."
    else
        echo "Error: Current user '$SOURCE_HOST' is missing some required privileges."
        return 1
    fi
...
check_if_source_mysql_is_empty() {
    ...
    OUTPUT=$(mysqlsh --uri "$DSN" --sql -e "SHOW DATABASES;" 2>/dev/null)
