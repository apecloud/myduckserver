...
// The below code lines are generated by Claude Sonnet 3.5
var (
...
func getRenamePgCatalogFuncRegex() *regexp.Regexp {
	initRenameMacroRegex.Do(func() {
		...
		renameMacroRegex = regexp.MustCompile(
			`(?i)(?:pg_catalog\.)?(?:"?(` + strings.Join(internalNames, "|") + `)"?)`)
	})
	return renameMacroRegex
}

func ConvertPgCatalogFuncToSys(sql string) string {
	return getRenamePgCatalogFuncRegex().ReplaceAllString(RemoveComments(sql), "__sys__.$1")
}

// Initializes and returns the regular expression for matching macros.
...
// Converts macro invocations in the given SQL string into the "(FROM ...)" format.
